...
import io.tapdata.flow.engine.V2.aspect.BatchReadNodeAspect;
import io.tapdata.flow.engine.V2.aspect.StreamReadNodeAspect;
import io.tapdata.flow.engine.V2.aspect.StreamReadStartedNodeAspect;
					...
					if (batchReadFunction != null) {
						for (String tableName : tapTableMap.keySet()) {
							...
							PDKInvocationMonitor.invoke(getConnectorNode(), PDKMethod.SOURCE_BATCH_READ,
									() -> batchReadFunction.batchRead(getConnectorNode().getConnectorContext(), tapTable, tableOffset, 100, (events, offsetObject) -> {
										if (events != null && !events.isEmpty()) {
											...
											InstanceFactory.instance(AspectManager.class).executeAspect(BatchReadNodeAspect.class, () -> new BatchReadNodeAspect().node(HazelcastSourcePdkDataNode.this).events(tapdataEvents));
					...
					() -> streamReadFunction.streamRead(getConnectorNode().getConnectorContext(), new ArrayList<>(tapTableMap.keySet()),
							syncProgress.getStreamOffsetObj(), 1, StreamReadConsumer.create((events, offsetObj) -> {
								if (events != null && !events.isEmpty()) {
									...
									InstanceFactory.instance(AspectManager.class).executeAspect(StreamReadNodeAspect.class, () -> new StreamReadNodeAspect().node(HazelcastSourcePdkDataNode.this).events(tapdataEvents));
								...
								if (null != newState && StreamReadConsumer.STATE_STREAM_READ_STARTED == newState) {
									InstanceFactory.instance(AspectManager.class).executeAspect(StreamReadStartedNodeAspect.class, () -> new StreamReadStartedNodeAspect().node(HazelcastSourcePdkDataNode.this));
									// MILESTONE-READ_CDC_EVENT-FINISH
