	...
	protected void doInit(@NotNull Context context) throws Exception {
		...
		for (LogCollecotrConnConfig logCollecotrConnConfig : logCollecotrConnConfigs) {
			...
			if (CollectionUtils.isNotEmpty(logCollecotrConnConfig.getNamespace())) {
				tableNamePrefix = ShareCdcUtil.joinNamespaces(logCollecotrConnConfig.getNamespace());
			}
			for (String tableName : logCollecotrConnConfig.getTableNames()) {
				HazelcastConstruct<Document> construct;
				if (null != tableNamePrefix) {
					construct = getConstruct(ShareCdcUtil.joinNamespaces(Arrays.asList(tableNamePrefix, tableName)), logCollecotrConnConfig.getConnectionId());
				}else{
