...
public abstract class PdkAutoInspectRunner extends AutoInspectRunner<IPdkConnector, IPdkConnector, IAutoCompare> {
    ...
    protected IPdkConnector openSourceConnector() throws Exception {
        ...
        return new PdkConnector(clientMongoOperator, taskId, node.getFromNode().getId(), getClass().getSimpleName() + "-" + node.getFromNode().getId(), conn, databaseType, this::isRunning, dataProcessorContext.getTaskConfig().getTaskRetryConfig());
    }
    ...
    protected IPdkConnector openTargetConnector() throws Exception {
        ...
        DatabaseTypeEnum.DatabaseType databaseType = ConnectionUtil.getDatabaseType(clientMongoOperator, conn.getPdkHash());
        return new PdkConnector(clientMongoOperator, taskId, node.getToNode().getId(), getClass().getSimpleName() + "-" + node.getToNode().getId(), conn, databaseType, this::isRunning, dataProcessorContext.getTaskConfig().getTaskRetryConfig());
    }
