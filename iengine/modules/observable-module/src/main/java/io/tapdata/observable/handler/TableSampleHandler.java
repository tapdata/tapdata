...
public class TableSampleHandler extends AbstractHandler {
    ...
    public Map<String, Map<String, Number>> retrieve(Map<String, String> tags, List<String> fields) {
    ...
    public void init(Node<?> node, Map<String, Long> tableCountMap) {
        Map<String, Map<String, Number>> tableValues = retrieve(baseTableTags(), Arrays.asList(
                "snapshotTotal", "snapshotInsertTotal"
        ));


        String nodeId = node.getId();
        // TODO(dexter): retrieve from db
        for(Map.Entry<String, Long> entry : tableCountMap.entrySet()) {
            ...
            SampleCollector collector = CollectorFactory.getInstance()
                    .getSampleCollectorByTags("tableSamplers", tags);
