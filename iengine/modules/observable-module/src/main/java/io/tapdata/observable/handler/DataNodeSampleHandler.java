...
public class DataNodeSampleHandler extends AbstractNodeSampleHandler {
    ...
    private final Map<String, CounterSampler> createTableCounters = new HashMap<>();
    ...
    public void init(Node<?> node, String associateId, Set<String> tables) {
        ...
        collector.addSampler("tableTotal", nodeTables::size);
        insertCounters.put(nodeId, collector.getCounterSampler("createTableTotal",
                values.getOrDefault("createTableTotal", 0).longValue()));
        updateCounters.put(nodeId, collector.getCounterSampler("snapshotTableTotal",
