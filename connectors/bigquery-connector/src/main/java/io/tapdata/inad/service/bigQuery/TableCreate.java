...
import io.tapdata.inad.util.bigQueryUtil.TableInfoDDL;
...
public class TableCreate extends BigQueryStart {
    ...
    public void createSchema(TapCreateTableEvent tapCreateTableEvent){
        ...
        if (io.tapdata.inad.util.tool.Checker.isEmptyCollection(nameFieldMap)){
...
//            }
            ...
            if (io.tapdata.inad.util.tool.Checker.isNotEmpty(defaultValue)) {
            ...
            sql.append(" OPTIONS (");
            if (io.tapdata.inad.util.tool.Checker.isNotEmpty(comment)) {
        ...
        if (io.tapdata.inad.util.tool.Checker.isNotEmpty(collateSpecification)){
    ...
    public void schemaListDDL(TapCreateTableEvent tapCreateTableEvent){
        ...
        BigQueryResult bigQueryResult = this.sqlMarker.executeOnce(sql);
        if (io.tapdata.inad.util.tool.Checker.isEmpty(bigQueryResult)){
        ...
        List<Map<String, Object>> result = bigQueryResult.result();
        if (io.tapdata.inad.util.tool.Checker.isEmptyCollection(result)){
