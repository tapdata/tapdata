...
public class Hive1Writer {
    ...
    //不支持带字段插入
//    protected static final String BATCH_INSERT_SQL = "INSERT INTO `%s`.`%s` (%s) VALUES ";
    protected static final String BATCH_INSERT_SQL = "INSERT INTO `%s`.`%s` VALUES ";

    private static final int MAX_BATCH_SAVE_SIZE = 10000;
    ...
    private void sumResult(WriteListResult<TapRecordEvent> writeListResult, WriteListResult<TapRecordEvent> result) {
    ...
    private WriteListResult<TapRecordEvent> batchInsert(TapConnectorContext tapConnectorContext, TapTable tapTable, List<TapRecordEvent> tapRecordEventList) throws Throwable {
        WriteListResult<TapRecordEvent> writeListResult = new WriteListResult<>(0L, 0L, 0L, new HashMap<>());
...
//        if(preparedStatement!=null) preparedStatement.clearBatch();
