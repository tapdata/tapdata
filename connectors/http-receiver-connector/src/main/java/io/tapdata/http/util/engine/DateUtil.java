...
import io.tapdata.util.CalendarUtil;
...
public class DateUtil {
	...
	public static Instant string2Instant(ZoneId zoneId, String dateString) {
		if (StringUtils.isBlank(dateString) || zoneId == null) {
	...
	public static Timestamp date2Timestamp(Date date, ZoneId toZoneId) throws Exception {
		if (date == null) {
			throw new NullPointerException();
		}
...
//    }
	...
	public static String microseondsToStringTime(Object data) {
		...
		if (passMidros != null) {
			...
			StringUtils.leftPad(ssssss, 6, "0");

			result.append(StringUtils.leftPad(hh, 2, "0")).append(":")
					.append(StringUtils.leftPad(mm, 2, "0")).append(":")
					.append(StringUtils.leftPad(ss, 2, "0")).append(".")
					.append(StringUtils.leftPad(ssssss, 6, "0"));
		}
		return result.toString();
	}

	/**
	 * 将从mongodb中的年份字符串转换成年份
	 *
	 * @param yearStr 年份字符串
	 * @return 年份对象
	 */
	public static MysqlYear string2MysqlYear(String yearStr) {
		String regexp = "^[1,2][0-9]{3}$";
		boolean matches = yearStr.matches(regexp);
		MysqlYear mysqlYear = new MysqlYear();
		if (matches) {
			int year = Integer.decode(yearStr);
