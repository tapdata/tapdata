...
public class ExpireHandel {
    ...
    public boolean refreshComplete(APIResponse tokenResponse,Map<String,Object> apiParam){
        ...
        if(Objects.nonNull(tokenName) && !"".equals(tokenName)) {
            ...
            if (split.length != 2){
                throw new CoreException(String.format("The wrong token rule description %s, please declare according to the correct rule, such as HttpResponseParam=PostManVariableParam. ",tokenName));
            }
        ...
        if (list.isEmpty()) {
            throw new CoreException("Not match any key, Unable to decide which to use as a valid value. Please manually set the global variable to which the obtained token is assigned on the connection page, such as HttpResponseParam=PostManVariableParam.");
        }
        ...
        if (list.size() == 1){
            ...
            throw new CoreException("Too many matches found. Unable to decide which to use as a valid value. Please manually set the global variable to which the obtained token is assigned on the connection page, such as HttpResponseParam=PostManVariableParam.");
        }
    }
    ...
    private Map.Entry<String, Object> fuzzyMatching(Map<String, Object> responseData){
        ...
        if (tokens.size() == 1){
            ...
            throw new CoreException("Too many matches found in variable map. Unable to decide which to use as a valid value. Please manually set the global variable to which the obtained token is assigned on the connection page, such as HttpResponseParam=PostManVariableParam.");
        }else {
            ...
            if (tokens.size() == 1){
                ...
                throw new CoreException("Not match any key in variable map., Unable to decide which to use as a valid value. Please manually set the global variable to which the obtained token is assigned on the connection page, such as HttpResponseParam=PostManVariableParam.");
            }
        }
    }
