package io.tapdata.connector.dameng.ddl;
...
public class DamengDDLSqlGenerator implements DDLSqlGenerator {
    ...
    public List<String> addColumn(CommonDbConfig config, TapNewFieldEvent tapNewFieldEvent) {
        ...
        for (TapField newField : newFields) {
            ...
            if (null != primaryKey && primaryKey) {
                TapLogger.warn(DamengDDLSqlGenerator.class.getSimpleName(), "Alter oracle table's primary key does not supported, please do it manually");
            }
        }
        return sqls;
    }
