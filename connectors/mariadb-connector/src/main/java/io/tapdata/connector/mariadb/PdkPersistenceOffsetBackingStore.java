...
public class PdkPersistenceOffsetBackingStore extends MemoryOffsetBackingStore {
	...
	private void load() {
		...
		MariadbStreamOffset mysqlStreamOffset = InstanceFactory.instance(JsonParser.class).fromJson(offsetStr, MariadbStreamOffset.class);
		String name = mysqlStreamOffset.getName();
		Map<String, String> offset = mysqlStreamOffset.getOffset();
		if (MapUtils.isEmpty(offset)) {
			return;
		}
