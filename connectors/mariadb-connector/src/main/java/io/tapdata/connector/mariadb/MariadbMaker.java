...
public class MariadbMaker implements SqlMaker {

	private static final String TAG = MariadbMaker.class.getSimpleName();
	...
	public String[] createTable(TapConnectorContext tapConnectorContext, TapCreateTableEvent tapCreateTableEvent, String version) throws Throwable {
		...
		String fieldSql = nameFieldMap.values().stream()
				.map(field -> {
					try {
						field.setDataType(MariadbUtil.fixDataType(field.getDataType(), version));
					} catch (Exception e) {
	...
	@Override
	public String selectSql(TapConnectorContext tapConnectorContext, TapTable tapTable, MariadbSnapshotOffset mariadbSnapshotOffset) throws Throwable {
		DataMap connectionConfig = tapConnectorContext.getConnectionConfig();
		String database = connectionConfig.getString("database");
		String tableId = tapTable.getId();
		String sql = String.format(MariadbContext.SELECT_TABLE, database, tableId);
		Collection<String> pks = tapTable.primaryKeys(true);
		List<String> whereList = new ArrayList<>();
		List<String> orderList = new ArrayList<>();
		if (MapUtils.isNotEmpty(mariadbSnapshotOffset.getOffset())) {
			for (Map.Entry<String, Object> entry : mariadbSnapshotOffset.getOffset().entrySet()) {
	...
	public String createIndex(TapConnectorContext tapConnectorContext, TapTable tapTable, TapIndex tapIndex) {
		...
		if (null == tapTable) {
			throw new IllegalArgumentException("Input parameter tapTable is null");
		}
		if (null == tapIndex) {
			throw new IllegalArgumentException("Input parameter tableIndex is null");
		}
		String database = tapConnectorContext.getConnectionConfig().getString("database");
		if (StringUtils.isBlank(database)) {
			throw new IllegalArgumentException("Database is blank");
		}
		String tableId = tapTable.getId();
		if (StringUtils.isBlank(tableId)) {
			throw new IllegalArgumentException("Table id is blank");
		}
		String indexType;
