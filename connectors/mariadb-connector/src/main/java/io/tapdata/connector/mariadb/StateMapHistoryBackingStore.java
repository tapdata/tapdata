...
public class StateMapHistoryBackingStore extends AbstractDatabaseHistory {
	...
	protected void storeRecord(HistoryRecord record) throws DatabaseHistoryException {
		...
		MariadbSchemaHistoryTransfer.executeWithLock(null, () -> {
			try {
				Document document = record.document();
				String hrJson = writer.write(document);
				Set<String> schemaSet = MariadbSchemaHistoryTransfer.historyMap.computeIfAbsent(serverName, k -> new LinkedHashSet<>());
				schemaSet.add(hrJson);
				MariadbSchemaHistoryTransfer.unSave();
			} catch (IOException e) {
	...
	protected void recoverRecords(Consumer<HistoryRecord> records) {
		Set<String> schemaSet = MariadbSchemaHistoryTransfer.historyMap.get(serverName);
	...
	public boolean exists() {
		return MariadbSchemaHistoryTransfer.historyMap.containsKey(serverName) && CollectionUtils.isNotEmpty(MariadbSchemaHistoryTransfer.historyMap.get(serverName));
	}
