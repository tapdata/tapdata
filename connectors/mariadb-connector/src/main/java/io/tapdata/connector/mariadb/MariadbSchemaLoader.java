...
public class MariadbSchemaLoader {
	private static final String TAG = MariadbSchemaLoader.class.getSimpleName();
	...
	private static  final String PRIMARYKEY_PREFIX="PRI";

	public MariadbSchemaLoader(MariadbContext mariadbContext) {
		this.mariadbContext = mariadbContext;
		this.tapConnectionContext = mariadbContext.getTapConnectionContext();
	}
	...
	private void discoverFields(List<DataMap> columnList, TapTable tapTable, TableFieldTypesGenerator tableFieldTypesGenerator,
		...
		columnList.forEach(dataMap -> {
			...
			field.nullable("YES".equals(isNullable));

			Object columnKey = dataMap.getObject("COLUMN_KEY");
			if (columnKey instanceof String && PRIMARYKEY_PREFIX.equals(columnKey)) {
	...
	private List<DataMap> queryAllColumns(String database, String tableNames) {
		...
		try {
			mariadbContext.query(sql, resultSet -> {
