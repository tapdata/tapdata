## **连接配置帮助**
### **1. POSTGRESQL安装说明**
请遵循以下说明以确保在 IKAS 中成功添加和使用PostgreSQL数据库。
### **2. 支持版本**
...
如果有多表合并同步的场景，则IKAS需要调整该属性为FULL
示例
```
...
#### **4.5 异常处理**
- **Slot清理**<br>
如果 IKAS 由于不可控异常（断电、进程崩溃等），导致cdc中断，会导致 slot 连接无法正确从 pg 主节点删除，将一直占用一个 slot 连接名额，需手动登录主节点，进行删除
查询slot信息
```
