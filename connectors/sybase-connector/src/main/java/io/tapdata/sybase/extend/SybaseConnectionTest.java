...
/**
 * @author GavinXiao
 * @description SybaseConnectionTest create by Gavin
 * @create 2023/7/10 18:49
 **/
public class SybaseConnectionTest extends CommonDbTest {
    protected static final String TAG = SybaseConnectionTest.class.getSimpleName();
    ...
    public SybaseConnectionTest(SybaseConfig mysqlConfig, Consumer<TestItem> consumer) {
        super(mysqlConfig, consumer);
        if (!ConnectionTypeEnum.SOURCE.getType().equals(commonDbConfig.get__connectionType())) {
            testFunctionMap.put("testWritePrivilege", this.ignoreTest("WritePrivilege"));
        }
        if (!ConnectionTypeEnum.TARGET.getType().equals(commonDbConfig.get__connectionType())) {
            testFunctionMap.put("testReadPrivilege", this.ignoreTest("ReadPrivilege"));
            testFunctionMap.put("testStreamRead", this.ignoreTest("StreamRead"));
        }
        jdbcContext = new SybaseContext(mysqlConfig);
        if (!ConnectionTypeEnum.SOURCE.getType().equals(commonDbConfig.get__connectionType())) {
            testFunctionMap.put("testCreateTablePrivilege", this::testCreateTablePrivilege);
            testFunctionMap.put("testWritePrivilege", this.ignoreTest("WritePrivilege"));
        }
        if (!ConnectionTypeEnum.TARGET.getType().equals(commonDbConfig.get__connectionType())) {
            testFunctionMap.put("testBinlogMode", this.ignoreTest("BinlogMode"));//this::testBinlogMode);
            testFunctionMap.put("testBinlogRowImage", this.ignoreTest("BinlogRowImage"));
            testFunctionMap.put("testCDCPrivileges", this.ignoreTest("CDCPrivileges"));
        }
    }
    ...
    public Boolean testCDCPrivileges() {
        ...
        try {
            ...
            List<SybaseConnectionTest.CdcPrivilege> cdcPrivileges = new ArrayList<>(Arrays.asList(SybaseConnectionTest.CdcPrivilege.values()));
            jdbcContext.normalQuery(getGrantsSql(), resultSet -> {
                while (resultSet.next()) {
                    String grantSql = resultSet.getString(1);
                    Iterator<SybaseConnectionTest.CdcPrivilege> iterator = cdcPrivileges.iterator();
                    while (iterator.hasNext()) {
                        boolean match = false;
                        SybaseConnectionTest.CdcPrivilege cdcPrivilege = iterator.next();
            ...
            if (null == testItem.get()) {
                if (CollectionUtils.isNotEmpty(cdcPrivileges) && cdcPrivileges.size() > 1) {
                    for (SybaseConnectionTest.CdcPrivilege cdcPrivilege : cdcPrivileges) {
