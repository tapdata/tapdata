...
public class OpenGaussOffsetBackingStore extends MemoryOffsetBackingStore {

    private OpenGaussOffset openGaussOffset;
    private WorkerConfig config;

    public OpenGaussOffsetBackingStore() {
    }
    ...
    public void configure(WorkerConfig config) {
        ...
        this.openGaussOffset = OpenGaussOffsetStorage.postgresOffsetMap.get(slotName);
    }
    ...
    private void load() {
        if (EmptyKit.isNull(openGaussOffset) || EmptyKit.isNull(openGaussOffset.getSourceOffset())) {
...
//            this.data.put(ByteBuffer.wrap(getOffsetKey().getBytes()), ByteBuffer.wrap(postgresOffset.getSourceOffset().getBytes()));
            this.data.put(ByteBuffer.wrap(getOffsetKey().getBytes()),
                    ByteBuffer.wrap(openGaussOffset.getSourceOffset().getBytes()));
        }
