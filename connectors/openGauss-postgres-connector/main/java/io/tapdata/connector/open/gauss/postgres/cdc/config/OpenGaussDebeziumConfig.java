package io.tapdata.connector.open.gauss.postgres.cdc.config;
...
public class OpenGaussDebeziumConfig {
    ...
    public OpenGaussDebeziumConfig() {

    }

    public OpenGaussDebeziumConfig use(OpenGaussConfig openGaussConfig) {
        this.openGaussConfig = openGaussConfig;
        return this;
    }

    public OpenGaussDebeziumConfig watch(List<String> observedTableList) {
    ...
    public OpenGaussDebeziumConfig useSlot(String slotName) {
...
//                .with("offset.storage.file.filename", "d:/cdc/offset/" + slotName + ".dat") //path must be changed with requirement
                ...
                .with("database.server.name", openGaussConfig.getDatabase())
                .with("database.hostname", openGaussConfig.getHost())
                .with("database.port", openGaussConfig.getPort())
                .with("database.user", openGaussConfig.getUser())
                .with("database.password", openGaussConfig.getPassword())
                .with("database.dbname", openGaussConfig.getDatabase())
                ...
                .with("plugin.name", openGaussConfig.getLogPluginName());
        if (EmptyKit.isNotEmpty(observedTableList)) {
            //construct tableWhiteList with schema.table(,) as <public.Student,postgres.test>
            String tableWhiteList = observedTableList.stream().map(v -> openGaussConfig.getSchema() + "." + v).collect(Collectors.joining(", "));
