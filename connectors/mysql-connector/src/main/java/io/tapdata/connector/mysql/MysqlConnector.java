...
public class MysqlConnector extends CommonDbConnector {
	...
	private Map<
			String, Object> filterTimeForMysql(ResultSet resultSet, ResultSetMetaData metaData,
											   Set<String> dateTypeSet) throws SQLException {
		...
		for (int i = 0; i < metaData.getColumnCount(); i++) {
			...
			try {
				...
				if ("TIME".equalsIgnoreCase(metaData.getColumnTypeName(i + 1))) {
					...
					try {value = resultSet.getObject(i + 1);
