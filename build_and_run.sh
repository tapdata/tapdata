...
# --- Function to read from .env safely ---
read_env() {
    local key=$1
    ...
    if [ -f .env ]; then
        # Handle CRLF and ensure we get the value after the first =
        value=$(grep "^${key}=" .env | tr -d '\r' | cut -d '=' -f2-)
    fi
...
WEB_PORT=$(read_env WEB_PORT 3030)
TM_PORT=$(read_env TM_PORT 3000)
MONGO_PORT=$(read_env MONGO_PORT 27017)
MONGO_USER=$(read_env MONGO_INITDB_ROOT_USERNAME admin)
MONGO_PWD=$(read_env MONGO_INITDB_ROOT_PASSWORD tapdata_best2019)

echo "----------------------------------------------------------------"
echo "Deployment Complete"
echo "Services:"
echo " - Web:    http://localhost:$WEB_PORT"
echo " - TM:     http://localhost:$TM_PORT"
echo " - Mongo:  mongodb://$MONGO_USER:$MONGO_PWD@localhost:$MONGO_PORT/tapdata"
echo "----------------------------------------------------------------"