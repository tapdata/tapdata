...
class WebHookControllerTest {
    ...
    class PingTest {
        ...
        void init() {
            ...
            when(webHookController.ping(webHookEvent)).thenCallRealMethod();
        }
        @Test
        void testPing() {
            try (MockedStatic<WebHookEvent> whe = mockStatic(WebHookEvent.class)) {
                whe.when(WebHookEvent::of).thenReturn(event);
                ResponseMessage<HookOneHistory> ping = webHookController.ping(webHookEvent);
                Assertions.assertNotNull(ping);
                Assertions.assertNotNull(ping.getData());
        ...
        void testFailUrl() {
            ...
            try (MockedStatic<WebHookEvent> whe = mockStatic(WebHookEvent.class)) {
                ...
                Assertions.assertThrows(BizException.class, () -> {
                    try {
                        webHookController.ping(webHookEvent);
