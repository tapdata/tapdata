...
import com.tapdata.tm.apiCalls.service.compress.CompressMinute;
import com.tapdata.tm.apiCalls.service.metric.Metric;
import com.tapdata.tm.apiCalls.service.metric.MetricRPS;
...
class WorkerCallServiceTest {
    ...
    @BeforeEach
    void init() {
        callService = mock(WorkerCallService.class);
    ...
    class metricTest {
        ...
        void testNormal() {
            ...
            map1.put("workers", info);
            Map<String, Object> workerInfo = new HashMap<>();
            workerInfo.put("oid", new ObjectId().toHexString());
            info.put("1", workerInfo);
            d4.setWorker_status(map1);
            apiServers.add(d4);
        ...
        @Test
        void testWorker_statusNotMap() {
            List<WorkerDto> apiServers = new ArrayList<>();

            WorkerDto d1 = new WorkerDto();
            d1.setProcessId("id");
            apiServers.add(d1);

            WorkerDto d2 = new WorkerDto();
            d2.setWorker_status(1);
            apiServers.add(d2);
        ...
        @Test
        void testWorkersNotMap() {
            List<WorkerDto> apiServers = new ArrayList<>();

            WorkerDto d3 = new WorkerDto();
            Map<String, Object> map = new HashMap<>();
            d3.setWorker_status(map);
            apiServers.add(d3);
