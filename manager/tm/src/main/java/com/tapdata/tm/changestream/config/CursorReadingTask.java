...
abstract class CursorReadingTask<T, R> implements Task {
	...
	private Supplier<Boolean> containerRunning;

	private final Log logger = LogFactory.getLog(CursorReadingTask.class);
	...
	CursorReadingTask(MongoTemplate template, SubscriptionRequest<?, ? super T, ? extends RequestOptions> request,
	                  Class<R> targetType, ErrorHandler errorHandler, Supplier<Boolean> containerRunning) {
	...
	public void run() {
		...
		if (containerRunning != null){
			while (containerRunning.get()){
				...
				logger.warn(String.format("Cursorreadingtask encountered an error. Try again in 500ms, collectionName: %s", collectionName));
				try {
					Thread.sleep(500);
