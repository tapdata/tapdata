...
public class TaskResetSchedule {
    ...
    public void checkNoResponseOp() {
        try {
            ...
            List<TaskResetLogs> taskResetLogs = taskResetLogService.find(query);

            if (CollectionUtils.isEmpty(taskResetLogs)) {
                return;
            }

            Map<String, TaskResetLogs> taskResetLogsMap = new HashMap<>();
            for (TaskResetLogs taskResetLog : taskResetLogs) {
                TaskResetLogs old = taskResetLogsMap.get(taskResetLog.getTaskId());
                if (old == null) {
            ...
            for (TaskResetLogs taskResetLog : taskResetLogs) {
                try {
                    ObjectId objectId = MongoUtils.toObjectId(taskResetLog.getId());
