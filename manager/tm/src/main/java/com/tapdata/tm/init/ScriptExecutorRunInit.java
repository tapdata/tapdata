	...
	private void executeScript(String path, String scriptVersionKey) throws IOException {
		...
		for (Resource resource : resources) {
			...
			for (Object script : scriptArray) {
				try {
					String oneScriptCommand = script.toString();
					for (Map.Entry<String, String> entry : replaceMap.entrySet()) {
						oneScriptCommand = oneScriptCommand.replaceAll(formatKey(entry.getKey()), entry.getValue());
					}
					Document document = mongoTemplate.executeCommand(oneScriptCommand);
					log.info("execute script: {} -- {}", oneScriptCommand, document.toJson());
				} catch (Exception e) {
