...
public class DAG implements Serializable, Cloneable {
    ...
    static {
        ...
        logger.warn("扫描到的 BeanDefinition 数量: " + result.size());
        result.forEach(beanDefinition -> {
            try {
                String className = beanDefinition.getBeanClassName();
                logger.warn("2. 正在处理类名: " + className);
                Class<?> nodeClass = Class.forName(beanDefinition.getBeanClassName());
                logger.warn("3. 类加载成功: " + nodeClass.getName());
                if (!Loader.isExtends(nodeClass, Node.class)) {
                    logger.warn("4. 继承检查失败，跳过: " + className); // 关键点4
                    return;
                }
                logger.warn("5. 继承检查通过: " + className); // 关键点5

                NodeType nodeType = nodeClass.getAnnotation(NodeType.class);
                logger.warn("6. 获取到的注解实例: " + nodeType); // 关键点6（如果是 null 就说明 Retention 不对）

                if (nodeType != null) {
                    nodeMapping.put(nodeType.value(), (Class<? extends Node>) nodeClass);
                    logger.warn("7. 成功放入映射，key: " + nodeType.value()); // 关键点7
                }
            } catch (Exception e) {
                logger.warn("8. 发生异常: " + e.getMessage()); // 关键点8
                e.printStackTrace();
