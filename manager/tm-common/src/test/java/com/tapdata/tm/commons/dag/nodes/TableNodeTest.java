...
//            doCallRealMethod().when(node).sortFieldsByPostion(schema);
...
//
//        @Test
//        void testMultiTableOnlyMatchedApplied() {
//            // 配置含 t1、t2 两表，当前 schema 为 t2，仅 t2 配置生效
//            Schema schema = schema("t2", "t2",
//                    field("a", "a", 1),
//                    field("b", "b", 2));
//            doSort(schema,
//                    fieldPostion("t1", postion("a", 1), postion("b", 2)),
//                    fieldPostion("t2", postion("a", 2), postion("b", 1)));
//            Assertions.assertEquals("b", schema.getFields().get(0).getFieldName());
//            Assertions.assertEquals("a", schema.getFields().get(1).getFieldName());
//        }
//    }
